...
const Notifications = () => {
  ...
  const fetchNotifications = async () => {
    try {
      ...
      const response = await axios.get('https://bus-project-qs4q.onrender.com/api/notifications');
  ...
  const handleSendClick = async () => {
    if (inputText.trim() !== "") {
      try {
        const response = await axios.post('https://bus-project-qs4q.onrender.com/api/notifications', {
          message: inputText
        });
